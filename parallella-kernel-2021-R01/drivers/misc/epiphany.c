...
#else
	rxcfg.remap_mode = 1;
//  Original ephipany 2019.1 address mapping
//	rxcfg.remap_sel = 0xfe0;
//	rxcfg.remap_pattern = 0x3e0;
//  Updated ephipany 2020.1 address mapping to support Epiphany GPIO
						...
						unsigned long arg)
{
    ...
    dev_info(&elink->dev, "elink_char_ioctl_elink_get_mappings: ********** Dump Mappings **********\n");
	ret = copy_to_user(dest, info, sizeof(*info));
	for(i=0;i<info->nmappings;i++)
	{
        dev_info(&elink->dev, "elink_char_ioctl_elink_get_mappings: map(%lld).emesh_addr = 0x%llx\n",i,info->mappings[i].emesh_addr);
        dev_info(&elink->dev, "elink_char_ioctl_elink_get_mappings: map(%lld).size = 0x%llx\n",i,info->mappings[i].size);
	}
    dev_info(&elink->dev, "elink_char_ioctl_elink_get_mappings: ***********************************\n");
				...
				unsigned long arg)
{
	...
	{
        printk(KERN_INFO "elink_char_ioctl: Exit-Bad Magic\n");
		return -ENOTTY;
	}

	if (_IOC_NR(cmd) > E_IOCTL_MAXNR)
	{
        printk(KERN_INFO "elink_char_ioctl: Exit failed-max exceed\n");
		return -ENOTTY;
	}
	...
	if (err)
	{
        printk(KERN_INFO "elink_char_ioctl: Exit failed-fault\n");
		return -EFAULT;
	}
				...
				unsigned long arg)
{
	...
	if (!array)
	{
        printk(KERN_INFO "mesh_char_ioctl: Exit failed-invalid\n");
		return -EINVAL;
	}
	...
	if (_IOC_TYPE(cmd) != E_IOCTL_MAGIC)
	{
        printk(KERN_INFO "mesh_char_ioctl: Exit failed-bad magic\n");
		return -ENOTTY;
	}

	if (_IOC_NR(cmd) > E_IOCTL_MAXNR)
	{
        printk(KERN_INFO "mesh_char_ioctl: Exit failed-max exceed\n");
		return -ENOTTY;
	}
	...
	if (err)
	{
        printk(KERN_INFO "mesh_char_ioctl: Exit failed-fault\n");
		return -EFAULT;
	}
					   ...
					   struct elink_device *elink)
{
	...
	for (; i > 0; i--) {
		...
		list_for_each_entry(region, &elink->mem_region_list, list) {
			...
			dev_info(&pdev->dev, "default_mappings: ***** Mapping *****\n");
			dev_info(&pdev->dev, "default_mappings: table map location = 0x%p\n",(void *) mapping);
			dev_info(&pdev->dev, "default_mappings: mapping->start = 0x%x\n",mapping->start);
			dev_info(&pdev->dev, "default_mappings: mapping->emesh_start = 0x%x\n",mapping->emesh_start);
			dev_info(&pdev->dev, "default_mappings: mapping->size = 0x%x\n",mapping->size);
			dev_info(&pdev->dev, "default_mappings: *******************\n");
				       ...
				       struct elink_device *elink)
{
	...
	dev_info(dev, "reserved_mem: res.start = 0x%x\n",res.start);
	dev_info(dev, "reserved_mem: res size = 0x%x\n",resource_size(&res));
    ...
    dev_info(dev, "reserved_mem: mem region successfully requested\n");
	...
	dev_info(dev, "reserved_mem: mem region successfully allocated\n");
	...
	dev_info(dev, "reserved-mem: added mem region at 0x%x\n", res.start);
